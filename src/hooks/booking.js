...
import { tourBookingService, hotelBookingService } from '../services';
...
export const useBookingToursData = (defaultParams = {}) => {
  const [bookings, setBookings] = useState([]);
  const [queryParams, setQueryParams] = useState(defaultParams);
  const [loading, setLoading] = useState(false);
  const [pagination, setPagination] = useState({
  ...
  const getTourBookings = async params => {
    try {
      setLoading(true);
      const res = await tourBookingService.getTourBookings(params);
      if (res.result) {
        const { tourBookings, total, page, pageSize } = res.result;
